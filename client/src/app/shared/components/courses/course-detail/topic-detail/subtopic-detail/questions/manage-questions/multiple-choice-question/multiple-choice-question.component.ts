import { Component, OnInit, Input, Inject, ViewChild,ElementRef,ViewContainerRef} from '@angular/core';
...
removeOptions() {
  if(this.optCount>1){
    const control = <FormArray>this.questionForm.controls['options'];
    control.removeAt(this.optCount);
    this.optCount--;
  }
}
/*
...
getFormValues() {
  ...
  return {
    ...
    options: options,
    qusType: CommonConfig.QUESTION_TYPE[1],
...
displayData(data:any) {
  let optionControls=[];
  this.optCount=data['options'].length-1;
